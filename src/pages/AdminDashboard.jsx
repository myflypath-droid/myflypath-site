...
export default function AdminDashboard() {
  ...
  const filteredOrders = clubFilter === "all"
    ? filteredByTime
    : filteredByTime.filter(o => o.club === clubFilter);
    ...
    <div className="min-h-screen font-sans px-6 py-8" style={{ background: '#13141f', color: '#fff' }}>
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <button onClick={() => fetchAll(password)}
        ...
        {/* Tabs */}
        <div className="flex gap-2 mb-8 p-1 rounded-2xl w-fit" style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.08)' }}>
          {[
            { id: "clubs", label: "Clubs" },
            { id: "codes", label: `Ventes (${usedCodes.length})` },
            { id: "stock", label: `Stock codes (${availableCodes.length})` },
          ].map((tab) => (
          ...
          <>
            ...
            <div className="rounded-2xl p-6 mb-8" style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)' }}>
              ...
              <form onSubmit={handleAddClub} className="flex gap-3 flex-wrap">
                ...
                <input type="text" value={newNom} onChange={(e) => setNewNom(e.target.value)}
                  placeholder="Nom affiche (ex: CapAero)" required
                  className="flex-1 min-w-40 rounded-xl px-4 py-2.5 text-sm text-white"
                  style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', outline: 'none' }}
                />
                <button type="submit" className="px-6 py-2.5 rounded-xl text-sm font-black"
            ...
            <div className="rounded-2xl overflow-hidden" style={{ border: '1px solid rgba(255,255,255,0.08)' }}>
              <table className="w-full">
                ...
                <tbody>
                  {clubs.length === 0 ? (
                    <tr><td colSpan={8} className="px-4 py-8 text-center text-sm" style={{ color: 'rgba(255,255,255,0.3)' }}>
                      Aucun club — ajoutez le premier !
                    </td></tr>
                  ) : clubs.map((club, i) => (
              ...
              <table className="w-full">
                ...
                <tbody>
                  {filteredOrders.length === 0 ? (
                    <tr><td colSpan={5} className="px-4 py-8 text-center text-sm" style={{ color: 'rgba(255,255,255,0.3)' }}>
        ...
        {activeTab === "stock" && (
          <div className="rounded-2xl p-8 text-center" style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)' }}>
            ...
            <p className="text-sm mt-2" style={{ color: 'rgba(255,255,255,0.3)' }}>{usedCodes.length} codes utilises sur {allCodes.length} au total</p>
            {availableCodes.length <= 10 && (
              <div className="mt-6 rounded-xl p-4" style={{ background: 'rgba(255,100,100,0.1)', border: '1px solid rgba(255,100,100,0.3)' }}>
                <p className="font-bold" style={{ color: '#FF6B6B' }}>Stock faible — pensez a recharger vos codes !</p>
              </div>
            )}
          </div>
        )}
